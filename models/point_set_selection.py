...
GRIPPER_TOP_DIR = "/scr2/MetaGrasp/Data/Gripper/Data_DB"
...
#### ListNet Loss
  ...
  train_op_stage2  = tf.train.AdamOptimizer(learning_rate=5e-5).minimize(loss_stage2)
...
def train(base=0):
  ...
  for epoch in range(FLAGS.num_epochs):
    ...
    for batch_id in range(int(num_batch)):
      ...
      #gripper_index = np.random.choice(np.array([3,5]),FLAGS.batch_size,replace=True) 
      ...
      for gj in range(FLAGS.batch_size): 
        stage1_top1 = out_single_point_top_index[gj][0] 
        stage1_top30 = out_single_point_top_index[gj][0:100] 

        stage1_acc_top1 += stage1_gt_gq[gj][stage1_top1]
        stage1_acc_top30 += (np.sum(stage1_gt_gq[gj][stage1_top30]))/100.0
      ...
      if 1:
       for gj in range(FLAGS.batch_size):
        ...
        pred_c = out_single_point_top_index[gj][0:50]
     ...
     #             print(np.sum(out_single_point_top_index[bi]))
        ...
        for gj in range(FLAGS.batch_size):
          ...
          stage2_acc_top1 +=  (np.sum(acc_map[0:1]))#(np.sum(acc_map[0:TOP_K]))/float(TOP_K)#acc_map[0]
          stage2_acc_top30 += (np.sum(acc_map[0:10])) > 0#(np.sum(weight_map[0:TOP_K]))/float(TOP_K)
        ...
        for gj in range(FLAGS.batch_size):
          ...
          top1_two_points_index_1 = top_two_points_index_1[0:10]
          top1_two_points_index_2 = top_two_points_index_2[0:10]
          ...
          gt_top1_two_points_index_1 = gt_top_two_points_index_1[0:20]
          gt_top1_two_points_index_2 = gt_top_two_points_index_2[0:20]
          ...
          if 0:
            ...
            c_pred[gt_top1_two_points_index_1,0] = 0.0
            c_pred[gt_top1_two_points_index_1,1] = 255.0
            c_pred[gt_top1_two_points_index_1,2] = 0.0
            c_pred[gt_top1_two_points_index_2,0] = 0.0
            c_pred[gt_top1_two_points_index_2,2] = 255.0
            showpoints(s_p,c_gt=c_pred,waittime=20,ballradius=4,freezerot=False) ## GRB

      # stage3 3
      if 0:
        gt_corr_label_list = []
        out_single_point_top_1024_index_list = []
        out_single_point_top_index_list = []
        out_two_points_top_index_list = []
        
        for bj in range(int(FLAGS.batch_size/gripper_size)):
          for ggi in range(gripper_size):
            gripper_id = gripper_index[ggi]
            ...
            out_single_point_top_index_list.append(np.load(s1_topk_dir))   

            s2_topk2_dir = os.path.join(env_save_dir,str(gripper_id)+'_s2_top1024.npy')  
            out_two_points_top_index_list.append(np.load(s2_topk2_dir))  

            s3_gt_dir = os.path.join(env_save_dir,str(gripper_id)+'_s3_gt.npy')
            gt_corr_label_list.append(np.load(s3_gt_dir))

        out_single_point_top_1024_index_s3 = np.array(out_single_point_top_1024_index_list).astype(np.int32)
...
#        print(np.sum(out_two_points_top_index_s3 == out_two_points_top_index)) 
        ...
        if 1:
         for gj in range(FLAGS.batch_size):
          ...
          two_points_label_ = out_two_points_top_index_s3[gj] 
          top_two_points_index_1_ = two_points_label_ // TOP_K2
          top_two_points_index_1_ = out_single_point_top_index_s3[gj][top_two_points_index_1_]
          top_two_points_index_2_ = out_single_point_top_1024_index_s3[gj][two_points_label_ % TOP_K2]
          
          top_f1_index_1 = top_two_points_index_1_[top_corr2_index]
          top_f2_index_2 = top_two_points_index_2_[top_corr2_index]
          top_f3_index_3 = out_single_point_top_1024_index_s3[gj][third_point_set_label % TOP_K2]
          ...
          gt_top_f3_index_3_ = out_single_point_top_1024_index_s3[gj][gt_three_points_label % TOP_K2]
          ...
          stage3_acc_top30 += (np.sum(gt_corr_label[gj][third_point_set_label[0:1]]) > 0)
         ...
         for gj in range(1):
          ...
          top1_f1_index_1 = top_f1_index_1[0:112]
          top1_f2_index_2 = top_f2_index_2[0:112]
          top1_f3_index_3 = top_f3_index_3[0:112]


          gt_three_points_label = np.where(gt_corr_label[gj] > 0)[0]
          num_gt_three = min(len(gt_three_points_label),110)
          gt_top_corr2_index = gt_three_points_label // TOP_K2

          gt_top_f1_index_1 = top_two_points_index_1_[gt_top_corr2_index]
          gt_top_f2_index_2 = top_two_points_index_2_[gt_top_corr2_index]
          gt_top_f3_index_3 = out_single_point_top_1024_index_s3[gj][gt_three_points_label % TOP_K2]
          ...
          if 1:
            ...
            showpoints(s_p,c_gt=c_pred,waittime=30,ballradius=4,freezerot=False) ## GRB
      ...
      if 0:
        ...
        for bbi in range(int(FLAGS.batch_size/gripper_size)):
          for ggi in range(gripper_size):
            ...
            bi = bbi * gripper_size + ggi
            env_i = str(train_val_test_list._train[bt_index[bbi]])
            env_dir = os.path.join('/juno/u/lins2/MetaGrasp/Data/BlensorResult', env_i)
            gripper_path = GRIPPER_TOP_DIR
...
#                print("gt_robotiq3f_path",gt_robotiq3f_path)
                gt_label = np.zeros((TOP_K, TOP_K2))
    ...
    stage1_acc_top30 /= (float(num_batch)*FLAGS.batch_size)
    stage2_acc_top1 /= (float(num_batch)*FLAGS.batch_size)
    stage2_acc_top30 /= (float(num_batch)*FLAGS.batch_size)
    stage3_acc_top1 /= (float(num_batch)*FLAGS.batch_size)
    stage3_acc_top30 /= (float(num_batch)*FLAGS.batch_size)
    gt_stage3_acc_top1 /= (float(num_batch)*FLAGS.batch_size)
    gt_stage2_acc_top1 /= (float(num_batch)*FLAGS.batch_size)
    ...
    print("stage1_top1_acc", stage1_acc_top1, " stage1_top30_acc:", stage1_acc_top30)
    print("stage2_top1_acc", stage2_acc_top1, " stage2_top30_acc:", stage2_acc_top30)
    print("stage3_top1_acc", stage3_acc_top1, " stage3_top30_acc:", stage3_acc_top30)
    print("gt_stage3_top1_acc", gt_stage3_acc_top1, " gt_stage3_top30_acc:", gt_stage2_acc_top1)
