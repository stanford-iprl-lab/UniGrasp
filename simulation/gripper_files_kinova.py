  ...
  def inverse_kinematic(self,target_tip_tf,chain_1,chain_2,chain_3,rot=False,initial_frame=None):
    ...
    if not rot:     
      ...
      def optimize_target(x):
        ...
        return dist3 * 0.0 + dist2 * 1. + dist1 * 1. + dist_rot1 * 0.01 + dist_rot2 * 0.01 + dist_rot3 * 0.00
...
if __name__ == "__main__":
  ...
  ### Gripper Model
  urdf_file = os.path.join(GRIPPER_DIR,"kinova_kg3_v2.urdf")
  ...
  f1_tip.tip_pc = np.array([0.03,-0.003,0.003]) 
  f3_tip.tip_pc = np.array([0.03,-0.003,0.01]) 
  f2_tip.tip_pc = np.array([0.03,-0.003,-0.01]) 
  ...
  target_frame_1 = f1_tip.forward_kinematics([0.1])
  target_frame_2 = f2_tip.forward_kinematics([0.1])
  ...
  #plot_pc(f2_tip_pc,color='green',mode='sphere',scale_factor=.0001)
