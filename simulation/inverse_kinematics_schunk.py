...
def inverse_kinematic_optimization(chain, target_frame, starting_nodes_angles, regularization_parameter=None,max_iter=None,rot=False):
  ...
  print("lenght of optimization variables",len(x))
  a = np.zeros(len(x),)
  b = np.zeros(len(x),)
  a[9] = 1
  a[18] = -1
  ...
  b[17] = -1
  #a[]
  linear_contraint = LinearContraint([a,b],[0,0],[0,0])
  # Utilisation 
  res = scipy.optimize.minimize(optimize_total, chain.active_from_full(starting_nodes_angles),method="L-BFGS-B",bounds=real_bounds, constrains=[linear_constraint]) 
  return res.x
