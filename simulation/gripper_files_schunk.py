...
#import inverse_kinematics_schunk as ik
from geolib.objfile import OBJ
...
class Gripper(object):
  ...
  def inverse_kinematic(self,target_tip_tf,chain_1,chain_2,chain_3,rot=False,initial_frame=None):
    ...
    b[13] = 1
    b[17] = -1
    linear_constraint = LinearConstraint([a,b],[0,0],[0,0])
  ...
  #      print("dist1",dist1,"distrot1",dist_rot1,"dist2",dist2,"dist_rot2",dist_rot2,"dist3",dist3,"dist_rot3",dist_rot3)
        return dist3 * 1.0 + dist2 * 1. + dist1 * 1. + dist_rot1 * 0.01 + dist_rot2 * 0.01 + dist_rot3 * 0.01
    ...
    for i in range(2):
      ...
      res = scipy.optimize.minimize(optimize_target, initial_pos, method="trust-constr",bounds=real_bounds,constraints=[linear_constraint])
      #print(res.fun)
      if res.fun < min_fun:
        min_fun = res.fun
        min_res_x = res.x
...
def finger_3f_initial_frame(p1,p2,p3,n1,n2,n3):
  ...
  for jj in range(3):
    gripper_center_1[jj] += n[0,jj] * 0.2
  gripper_frame[:3,2] = -n
  ...
  print("gripper_frmae")
  gripper_frame[:3,1] = n1 * -1.0
  gripper_frame[:3,0] = np.cross(gripper_frame[:3,1],gripper_frame[:3,2])
  gripper_frame[:3,0] = -1.0 * gripper_frame[:3,0] / (np.linalg.norm(gripper_frame[:3,0]) + 1e-16)
...
if __name__ == "__main__":
  ...
  ### Gripper Model
  urdf_file = os.path.join(GRIPPER_DIR,"schunk_right_hand_v2.urdf")
  with open(urdf_file,'r') as myfile:
    urdf_strings =  myfile.read().replace('\n','')
  gripper_in_urdf = URDF.from_xml_string(urdf_strings)

  tip_pc = np.array([0.0,-0.00,0])
  gripper = Gripper(gripper_in_urdf=gripper_in_urdf,tip_pc=tip_pc)

  gripper.get_root()
  #gripper.get_all_T0()
   #### IK test
  f1_tip = gripper.chain_map['finger_tip_1']
  f2_tip = gripper.chain_map['finger_tip_2']
  f3_tip = gripper.chain_map['finger_tip_3']

  f1_tip.tip_pc = np.array([0.028,0.00,-0.00]) 
  f3_tip.tip_pc = np.array([0.022,0.00,-0.00]) 
  f2_tip.tip_pc = np.array([0.022,0.00,-0.00]) 

  #################################
  #   Set the base frame 
  ################################# 
  target_base_frame = np.eye(4)
  #target_base_frame[:3,-1] = np.array([0.00,-0.00,0.2])
...
#  mayalab.show()
  ...
  spread = 0.0
  #############################
  #  f1_tip order is [j4:revol, j3:revol, thum_flexion:revol, thum_help: fixed, thum_opposition: revol, f4 fixed]
  ###############################
  for joint in f1_tip.joints:
    print(joint.name)
  print(f1_tip.active_joints_mask)

  q1_list = [0.1,0.1,0.1,oppo]
  target_frame_1 = f1_tip.forward_kinematics(q1_list)
  count = 0
  for idx,joint in enumerate(f1_tip.joints):
    if f1_tip.active_joints_mask[idx]:
      gripper.q_value_map[joint.name] = q1_list[count]
      print(joint.name,q1_list[count],count)
      count  = count + 1


  ##############################################
  #  f2_tip_order is [j14:revol, Distal: revol, Proximal: revol, spread: revol] 
  ###########################################################
  print("f2_tip_order %%%%%%%%%%%%%%%%%%%%")
  print(f2_tip.joints)

  for joint in f2_tip.joints:
    print(joint.name)
  print(f2_tip.active_joints_mask)
  q2_list = [0.35,0.175,0.,spread]
  target_frame_2 = f2_tip.forward_kinematics(q2_list)
  print("target_frame_2",target_frame_2)
  count = 0
  for idx,joint in enumerate(f2_tip.joints):
    if f2_tip.active_joints_mask[idx]:
      gripper.q_value_map[joint.name] = q2_list[count]
      print(joint.name,q2_list[count],count)
      count  = count + 1


  ##############################################
  #  f3_tip_order is [j16:revol, j12:revol, Finger: revol, spread: revol, $j4: thum_opposition] 
  ###########################################################
  print("%%%%%%%%%%%%%%%%%%%%")
  print(f3_tip.joints)

  for joint in f3_tip.joints:
    print(joint.name)
  print(f3_tip.active_joints_mask)
  q3_list = [0.1,0.1,0.1,spread,oppo]
  target_frame_3 = f3_tip.forward_kinematics(q3_list)
  count = 0
  for idx,joint in enumerate(f3_tip.joints):
    if f3_tip.active_joints_mask[idx]:
      gripper.q_value_map[joint.name] = q3_list[count]
      print(joint.name,q3_list[count],count)
      count  = count + 1
...
# Set the target_tip 
###########################
  env_dir = '/home/lins/MetaGrasp/Data/BlensorResult/473'
  ...
  print(obj_pcs.shape)
#  plot_pc_with_normal(obj_pcs[:,:3],obj_pcs[:,3:]*0.01)

  idx1 = 1000
  idx2 = 520
  idx3 = 10

  target_tip_1[:3,3] = obj_pcs[idx1,:3]
  target_tip_2[:3,3] = obj_pcs[idx2,:3]
  target_tip_3[:3,3] = obj_pcs[idx3,:3]

  target_tip_1[:3,2] = obj_pcs[idx1,3:]
  target_tip_2[:3,2] = obj_pcs[idx2,3:]
  target_tip_3[:3,2] = obj_pcs[idx3,3:]

  target_tip_1[:3,0] = np.array([0,0,1])
  target_tip_2[:3,0] = np.array([0,0,1])
  target_tip_3[:3,0] = np.array([0,0,1])

  target_tip_1[:3,1] = np.cross(target_tip_1[:3,2],target_tip_1[:3,0])
  target_tip_2[:3,1] = np.cross(target_tip_2[:3,2],target_tip_2[:3,0])
  target_tip_3[:3,1] = np.cross(target_tip_3[:3,2],target_tip_3[:3,0])

  target_tip_1 = np.load('tip1_frame.npy')
  target_tip_2 = np.load('tip2_frame.npy')
  target_tip_3 = np.load('tip3_frame.npy')

  f1_tip_pc = target_tip_1[:3,3]
  f1_tip_pc = np.array(f1_tip_pc).reshape((-1,3))
  f1_tip_n = np.copy(target_tip_1[:3,2])
  f1_tip_n = np.array(f1_tip_n).reshape((-1,3))
  #plot_pc(f1_tip_pc,color='ycan',mode='sphere',scale_factor=.01)
  ...
  f2_tip_n = np.copy(target_tip_2[:3,2])
  ...
  f3_tip_n = np.copy(target_tip_3[:3,2])
  ...
  initial_frame = finger_3f_initial_frame(f1_tip_pc,f2_tip_pc,f3_tip_pc,f1_tip_n,f2_tip_n,f3_tip_n)
  ...
  if 1: 
    q_list, mnist = gripper.inverse_kinematic(target_tips,gripper.chain_map['finger_tip_1'],gripper.chain_map['finger_tip_2'],gripper.chain_map['finger_tip_3'],rot=False,initial_frame=initial_frame)
    print(q_list)
    print("length of q_liust", len(q_list))
    print("error",mnist)
    ...
    #    count  = count + 1
    predict_f1_tip = gripper.link_map['base_link'].base_T.dot(gripper.chain_map['finger_tip_1'].forward_kinematics([q_list[6:10]]))
    predict_f1_tip = np.array(predict_f1_tip[:3,-1]).reshape((-1,3))
    plot_pc(predict_f1_tip,color='ycan',mode='cube',scale_factor=.005)
  ...
  #      count  = count + 1
    ...
    predict_f2_tip = gripper.link_map['base_link'].base_T.dot(gripper.chain_map['finger_tip_2'].forward_kinematics([q_list[10:14]]))
    ...
    for idx,joint in enumerate(f3_tip.joints):
      if f3_tip.active_joints_mask[idx]:
        ...
        print(joint.name,q_list[count],count)
        count  = count + 1
    predict_f3_tip = gripper.link_map['base_link'].base_T.dot(gripper.chain_map['finger_tip_3'].forward_kinematics([q_list[14:]]))
