...
class Visual(object):
  def __init__(self,visual_in_urdf):
    ...
    self.obj.pc_4d = np.hstack([self.obj.pc,np.ones((self.obj.len_pc,1))]).dot(self.T.transpose())
    self.obj.pc_T = self.obj.pc_4d[:,0:3]
    print("T",self.T)
...
class Gripper(object):
  def __init__(self,gripper_in_urdf,tip_pc):
    ...
    for elem in self.joints:
      if elem.type == 'revolute' or elem.type == 'prismatic':
        num_j = num_j + 1
    self.permutation_matrix = np.zeros((2 ** num_j, num_j))

    for jj in range(num_j):
      for ii in range( 2 ** num_j):
        if ( (ii) % ( 2 ** (jj+1) ) + 1 ) > (2 ** jj):
          self.permutation_matrix[ii,jj] = 1.0


    for count in range( 2 ** num_j):
     idx = 0
     for elem in self.joints:
      if elem.type == 'revolute' or elem.type == 'prismatic':
        self.q_limit_map[elem.name] = elem.limit
        if self.permutation_matrix[count][idx]:
          self.q_value_map[elem.name] = np.random.uniform(low=elem.limit.upper,high=elem.limit.upper,size=1)[0]
        else:
          self.q_value_map[elem.name] = np.random.uniform(low=elem.limit.lower,high=elem.limit.lower,size=1)[0]
  ...
  def vis(self,save=False,idd=0):
    self.get_all_T0()
    self.pc_whole = []
    print("len of link_map",len(self.link_map))
    for link_name in self.link_map:
      if link_name in self.parent_map:
        print("idd",link_name,self.T0_map[link_name])
    ...
    save_dir = '/home/lins/MetaGrasp/Data/Gripper/Data'
    if save:
      np.save(os.path.join(save_dir,'sawyer_'+str(idd)+'.npy'),self.pc_whole)
  ...
  def inverse_kinematic(self,target_tip_tf,chain_1,chain_2,chain_3,rot=False,initial_frame=None):
    ...
    if not rot:     
      ...
      def optimize_target(x):
        ...
        return dist3 * 1.25 + dist2 * 0.5 + dist1 * 1.25 + dist_rot1 * 0.01 + dist_rot2 * 0.01 + dist_rot3 * 0.01
...
if __name__ == "__main__":
  ### Gripper Model
  urdf_file = os.path.join(GRIPPER_DIR,"sawyer_hand.urdf")
