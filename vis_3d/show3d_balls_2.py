...
def showpoints(xyz,c_gt=None, c_pred = None ,waittime=0,showrot=False,magnifyBlue=0,freezerot=True,background=(255,255,255),normalizecolor=False,ballradius=5):
    ...
    c01 = np.expand_dims(c0,axis=-1)
    c11 = np.expand_dims(c1,axis=-1)
    c21 = np.expand_dims(c2,axis=-1)
    #print(np.hstack([c01,c11,c21]))
...
if __name__=='__main__':
    c_ = np.abs(np.random.randn(500,1))
    c_1 = np.ones_like(c_) * 255.0
    c_0 = np.zeros_like(c_) 
    c_gt = np.hstack([c_0,c_1,c_0]) # GRB
    np.random.seed(100)
    showpoints(np.random.randn(500,3) * np.array([10.0,10.0,0.0]),c_gt=c_gt)
